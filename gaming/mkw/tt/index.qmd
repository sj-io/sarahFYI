...
all_tt <- tt |> 
  ...
  mutate(time_pr = period_to_seconds(time_pr),
         time_wr = period_to_seconds(time_wr),
         diff = seconds_to_period(time_pr - time_wr) |> round(3),
         date_pr = as_date(date_pr)) |> 
  arrange(diff)

attempts <- tt |> 
  summarise(attempts = n(), .by = "track")
...
pb_tbl <- best_tt |> 
  ...
  left_join(attempts) |> 
  mutate(across(starts_with("time"), ~ seconds_to_period(.) |> as.character()))
...
reactable(pb_tbl,
  ...
  columns = list(
    ...
    attempts = colDef(name = "attempts", maxWidth = 70)
  ),
  columnGroups = list(
...
::: {.card title="all WRs"}
```{r}
wr_tbl <- wr |> select(-label) |> mutate(time = as.character(time))
...
all_dates <- seq.Date(min(daily_tt$date), Sys.Date(), by = "day") |> tibble::as_tibble_col("date")
all_dates <- all_dates |> left_join(daily_tt) |> mutate(n = replace_na(n, 0))

gg_tt_count <- all_dates |> 
  ggplot() +
...
not_pr <- all_tt_sec |> anti_join(best_tt) |> filter(track != "rainbow road")
gg_wr <- ggplot() +
  geom_point(
    data = not_pr, 
  ...
  geom_point(
    data = best_tt |> filter(track != "rainbow road"), 
    aes(y = track, x = time_pr, text = label_pr), 
    color = "gold") +
  # geom_violin(
  #   data = all_tt_sec, 
