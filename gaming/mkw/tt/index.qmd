...
```{r}
knitr::kable(best_tt |> select(track, starts_with("date"), starts_with("time"), diff))
```
...
gg_wr <- ggplot() +
  geom_point(data = all_tt_sec |> anti_join(best_tt_sec), aes(x = track, y = time_pr, text = label_pr), color = "grey40") +
  geom_point(data = best_tt_sec, aes(x = track, y = time_pr, text = label_pr), color = "gold") +
  # geom_violin(
  #   data = all_tt_sec, 
